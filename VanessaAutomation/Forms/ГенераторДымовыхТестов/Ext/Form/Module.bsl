...
&НаКлиентеНаСервереБезКонтекста
Функция ПолучитьПредставлениеТипаМетаданных(Язык, ПоПравиламИмен = Ложь)
	
	ВозвращемыеДанные = Новый  Структура();
	
	Если Язык = "en" Тогда 
		Если ПоПравиламИмен Тогда 
...
Процедура ФормированиеФайловТестов()
	...
	СтруктураПараметров.Вставить("ПредставлениеТипаМетаданныхКакИмя", ПолучитьПредставлениеТипаМетаданных(ЯзыкШагов, Истина));
...
Процедура ДобавитьТекстРасширенныеДействия(Текст, Параметры, ОбъектМетаданных, Группа = Ложь)
	...
	//	
	//	ЗапросНаПолучениеСсылки = "SELECT TOP 1 CurrentDataSource.Ref AS Ref FROM " + Параметры.ПредставлениеТипаМетаданныхКакИмя[Параметры.ВидОбъектаМетаданных]+ "." + ОбъектМетаданных.Имя + " AS CurrentDataSource WHERE Not CurrentDataSource.DeletionMark " + ТекстУсловие;
	//	
	//	Текст.ДобавитьСтроку("
	...
	//	|			| ""Query = Новый Query;""								|
	//	|			| ""Query.Text = """""+ЗапросНаПолучениеСсылки +  """; ""|" + "            
	...
	//	|			And Given I open hyperlink  '$НавигационнаяСсылка$'
	//	|			If the warning is displayed then
	//	|				Then I raise ""Could not open existing item" + ПредставлениеОбъекта + """ exception" + "
	//	|			If current form name is ""ErrorWindow"" Then
	//	|				Then I raise ""Could not open existing item " + ПредставлениеОбъекта + """ exception" + "
	//	|			And I close the form named 'ЗаголовокФормы'");		
	//КонецЕсли;
