...
Function ПолучитьПредставлениеТипаМетаданных()
	ВозвращемыеДанные = Новый  Структура();
	
	If ЯзыкШагов = "en" Then
...
Процедура СформироватьФайлы(Команда)
	...
	Для Каждого СтрокаНастройки ИЗ ТаблицаНастроек Цикл
		...
		НайденныеФайлы = НайтиФайлы(КаталогФайловИсключений, СтруктураПараметров.ВидОбъектаМетаданных + "*.txt", Истина);
		Для Каждого НайденныйФайл Из НайденныеФайлы Цикл	
			...
			КонецЦикла;
		КонецЦикла;
		СтруктураПараметров.Вставить("МассивИсключений", МассивИсключений);
		...
		Для СчТип = 1 По СоотвестствиеНастроек.Количество() Цикл
			...
			Если СтрокаНастройки[ТипТеста] Тогда
				...
				Если Текст.КоличествоСтрок() > 0 Тогда
					...
					// Запишем файл
					Попытка
						Текст.Записать(КаталогВыходныхФайлов + "\" + СчОбъект + СчТип + "_" + СтруктураПараметров.ВидОбъектаМетаданных + "_" + ТипТеста + ".feature", КодировкаТекста.UTF8);
					Исключение
