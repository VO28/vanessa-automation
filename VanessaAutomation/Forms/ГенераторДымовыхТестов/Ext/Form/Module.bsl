...
Процедура СформироватьФайлы(Команда)
	...
	ОчиститьСообщения();
	Сообщить("Начало формирования тестов: " + ТекущаяДата());
...
Процедура ФормированиеФайловТестов()
	...
	Сообщить("Окончание формирования тестов: " + ТекущаяДата() + " (" + (ТекущаяУниверсальнаяДатаВМиллисекундах() - ВремяНачала)/1000 + " сек.)");
...
Процедура ЗагрузитьНастройкиСценариевЗавершение(Результат, Адрес, ВыбранноеИмяФайла, ДополнительныеПараметры) Экспорт
	...
	Если Результат Тогда
		...
		Состояние(Нстр("ru = 'Выполняется настройки сценариев. Пожалуйста, подождите...'"));
