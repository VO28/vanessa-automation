...
Функция СформироватьТекстФайлаПоОбъектамМетаданных(Параметры)
	...
	Если Текст.КоличествоСтрок() > 0 Тогда
		...
		//	|@tree
		//	|
		//	|Functionality: Smoke tests - " + Параметры.ПредставлениеТипаМетаданных[Параметры.ВидОбъектаМетаданных] + " - " + Параметры.ПредставлениеТипТеста[Параметры.ТипТеста] + "
...
Процедура ДобавитьТекстОткрытияФормыНового(Текст, Параметры, ОбъектМетаданных)
	...
	//	|	Дано Я открываю основную форму " + Параметры.ВидОбъектаРП + " """ + ОбъектМетаданных.Имя + """
	//	|	If the warning is displayed then
	//	//|		Then I raise ""The main form could not be opened " + Параметры.ВидОбъектаРП + " " + ОбъектМетаданных.Имя + """ exception"
	//	|	If current form name is ""ErrorWindow"" Then
	//	//|		Then I raise ""The main form could not be opened " + Параметры.ВидОбъектаРП + " " + ОбъектМетаданных.Имя + """ exception"
	//	|	And I close current window");	
	//КонецЕсли;
